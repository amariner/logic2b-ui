...
There is no stable (`latest`) release yet; `1.0.0` will be the first. The beta
is the `next` channel for both npm packages and the registry. Versions below
are the ones this documentation was verified against.
...
| CLI and local MCP packages | `logic2b@next`, `@logic2b/mcp@next` (published `1.0.0-rc.2`) | Node.js 18+ with npm, pnpm, yarn or bun |
