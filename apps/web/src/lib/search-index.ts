...
const EXTRA: SearchIndexItem[] = [
  ...
  {
    title: "Live Starter Demos",
    description:
      "Run the complete marketing, dashboard and authentication starters generated by the CLI and MCP scaffold tools.",
    url: "/demos",
    group: "Guides",
  },
];
...
export async function getSearchIndex(): Promise<SearchIndexItem[]> {
  ...
  const docItems: SearchIndexItem[] = docs.map((d) => ({
    ...
    url: d.id === "index" ? "/docs" : `/docs/${d.id}`,
    group: d.id.startsWith("components/") ? "Components" : "Guides",
  }));
  const spanishDocItems: SearchIndexItem[] = docsEs.map((d) => ({
    title: `${d.data.title} · ES`,
    description: d.data.description,
    url: d.id === "index" ? "/es/docs" : `/es/docs/${d.id}`,
    group: d.id.startsWith("components/") ? "Components" : "Guides",
  }));
  ...
  return [...docItems, ...spanishDocItems, ...blockItems, ...chartItems, ...EXTRA].sort((a, b) => {
