/**
 * Theme presets for the MCP theme tools (`get_theme`, `decode_preset`,
 * `apply_preset`).
 *
 * A preset id is base64url("base|theme|chart|radius|font|heading") — the same
 * format /create generates and `logic2b init --preset` consumes. This module
 * mirrors the data in packages/cli/src/themes.ts (itself a mirror of
 * apps/web/src/lib/themes.ts); test/themes.test.ts fails if the two drift.
 * Extracting a shared @logic2b/tokens package is on the roadmap.
 *
 * Portability note: this file runs both in Node (stdio server) and inside the
