...
import { encodePreset, FONTS, ICON_LIBRARIES, RADII, type ThemeConfig } from "@logic2b/tokens"
...
function inventory(items: readonly IndexEntry[]) {
...
export function buildAgentsMd(cfg: ThemeConfig, items: readonly IndexEntry[]): string {
  const preset = encodePreset(cfg)
  const { components, blocks, charts } = inventory(items)
